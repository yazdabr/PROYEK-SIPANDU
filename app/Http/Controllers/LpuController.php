...
class LpuController extends Controller
{
    ...
    public function create()
    {
        ...
        $unitLpu = DB::table('unit_pengolah')->where('nama_unit', 'LPU')->first();

        return view('uplpu.lpuinput', compact('kodeKlasifikasi', 'unitLpu'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('lpu.index')->with('success', 'Arsip berhasil disimpan!');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('lpu.index')->with('success', 'Data berhasil dihapus');
    }
    ...
    public function edit($id)
    {
        ...
        $lpu = ArsipUnit::findOrFail($id);
        $kodeKlasifikasi = KodeKlasifikasi::all();

        $unitLpu = DB::table('unit_pengolah')->where('nama_unit', 'LPU')->first();
        if (!$unitLpu) {
            abort(500, 'Unit LPU tidak ditemukan di database.');
        }

        return view('uplpu.lpuedit', compact('lpu', 'kodeKlasifikasi', 'unitLpu'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $lpu->judul                = $validated['judul'];
        $lpu->nomor_arsip          = $nomor;
        $lpu->kode_klasifikasi_id  = (int) $validated['kode_klasifikasi'];
        $lpu->kategori             = $kategori;
        $lpu->kategori_berita      = $validated['kategori_berita'];
        $lpu->indeks               = $validated['indeks'] ?? null;
        $lpu->uraian_informasi     = $validated['uraian_informasi'] ?? null;
        $lpu->tanggal              = $validated['tanggal'] ?? null;
        $lpu->tingkat_perkembangan = $validated['tingkat_perkembangan'];
        $lpu->jumlah               = $validated['jumlah'] ?? null;
        $lpu->satuan               = $validated['satuan'] ?? null;
        $lpu->unit_pengolah_id     = $validated['unit_pengolah_id'];
        $lpu->ruangan              = $validated['ruangan'] ?? null;
        $lpu->no_box               = $validated['no_box'] ?? null;
        $lpu->no_filling           = $validated['no_filling'] ?? null;
        $lpu->no_laci              = $validated['no_laci'] ?? null;
        $lpu->no_folder            = $validated['no_folder'] ?? null;
        $lpu->skkaad               = $validated['skkaad'] ?? null;
        $lpu->keterangan           = $validated['keterangan'] ?? null;

        if ($request->hasFile('upload_dokumen')) {
            $lpu->upload_dokumen = $request->file('upload_dokumen')->store('arsip', 'public');
        }

        $lpu->save();

        return redirect()->route('lpu.index')->with('success', 'Data berhasil diperbarui');
    }
}
