...
>
    ...
    <div class="px-3 py-2 bg-[#68778B] flex items-center justify-between shadow-md">
        ...
        <nav class="flex-1 p-4 space-y-2">
            ...
            <a href="{{ url('/ppid/verifikasi') }}" class="group flex items-center space-x-2 p-3 rounded hover:bg-[#CBD2DA] transition-all duration-300 ease-in-out font-semibold">
                <img src="/images/verif.png" alt="" class="w-7 h-7 transition-transform duration-300 group-hover:scale-110">
                <span class="transition-transform duration-300 group-hover:translate-x-1">Verifikasi</span>
            </a>
            <a href="{{ url('/ppid/dap') }}" class="group flex items-center space-x-2 p-3 rounded hover:bg-[#CBD2DA] transition-all duration-300 ease-in-out font-semibold text-[#003B69]">
        ...
        <main class="p-4 md:p-6 space-y-6 flex-1 overflow-y-auto" x-data="{ showModal: false, selected: null }">
            ...
            <div class="bg-white p-4 md:p-5 rounded shadow">
                ...
                <form action="{{ url()->current() }}" method="GET" class="mb-4 flex flex-wrap items-center gap-2">
                        ...
                        <select id="judulSelect" name="judul" class="w-full border rounded px-3 py-2 text-gray-700">
                            ...
                            @if(isset($judulList))
                                @foreach($judulList as $judul)
                                    <option value="{{ $judul }}" {{ request('judul') == $judul ? 'selected' : '' }}>{{ $judul }}</option>
                                @endforeach
                            @else
                                @foreach($arsipPublik as $arsip)
                                    <option value="{{ $arsip->id }}" {{ request('judul') == $arsip->id ? 'selected' : '' }}>{{ $arsip->judul }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    ...
                    <button type="button" onclick="window.location.href='{{ url('/ppid/dap') }}'" class="bg-gray-400 text-white px-3 py-2 rounded hover:bg-gray-500 flex-shrink-0">Reset</button>
                </form>
                                ...
                                <td class="px-3 py-2 border text-center sticky-action-column">
                                    <div class="flex justify-center items-center space-x-2">
                                        ...
                                        <form action="{{ route('arsip.publik.hapus', $item->id) }}" method="POST" class="inline deleteForm">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="deleteBtn bg-[#BB5456] text-white p-1 rounded hover:bg-[#8B6869] transition font-semibold" title="Hapus">
    ...
    <!-- Global toast (blade session) -->
    @if(session('success'))
    <div id="toast" class="fixed bottom-5 left-1/2 transform -translate-x-1/2 z-50">
        <div class="bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg opacity-0">
            {{ session('success') }}
        </div>
    </div>
    @endif

    <!-- Delete modal (single, outside loop) -->
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-[99999] opacity-0 transition-opacity duration-300">
        <div id="modalContent" class="bg-white p-6 rounded-lg shadow-lg w-11/12 max-w-sm text-center transform scale-90 opacity-0 transition-all duration-300">
