...
<head>
    ...
    <style>
        ...
        /* small utility for toast */
        #toast > div { transition: opacity .5s ease; }
    </style>
</head>
...
>
    <div class="px-3 py-2 bg-[#68778B] flex items-center justify-between shadow-md">
        ...
        <nav class="flex-1 p-4 space-y-2">
            ...
            <a href="{{ url('/manajemen/laporanarsip') }}" class="group flex items-center space-x-2 p-3 rounded hover:bg-[#CBD2DA] transition-all duration-300 ease-in-out font-semibold">
            ...
            <a href="{{ url('/manajemen/laporanlayanan') }}" class="group flex items-center space-x-2 p-3 rounded hover:bg-[#CBD2DA] transition-all duration-300 ease-in-out font-semibold text-[#003B69]">
            ...
            <div class="bg-white p-4 md:p-5 rounded shadow">
                <h2 class="font-bold text-lg text-[#003B69] mb-4">Daftar Arsip Publik</h2>
                    ...
                    <form action="{{ url()->current() }}" method="GET" class="mb-4 flex flex-wrap items-center gap-2">
                        ...
                        <button type="button" onclick="window.location.href='{{ url('/manajemen/laporanlayanan') }}'" class="bg-gray-400 text-white px-3 py-2 rounded hover:bg-gray-500 flex-shrink-0">
                            Reset
                        </button>
                    </form>
                ...
                <div class="overflow-x-auto max-h-[70vh] md:max-h-[56vh] border border-gray-200 rounded">
                    <table class="hidden md:table min-w-full text-sm">
                        ...
                        <tbody>
                            {{-- Perhatikan, perulangan ini menggunakan $arsipPublik, pastikan variabel ini tersedia di controller Anda --}}
                            ...
                            @empty
                                {{-- Baris kosong untuk data desktop --}}
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <div class="md:hidden p-2 space-y-3">
                        ...
                        <div class="bg-white p-3 rounded-lg shadow border border-gray-200">
                            ...
                            <div class="flex justify-end items-center space-x-2 mt-3 mobile-actions-container">
                                ...
                                <form action="{{ route('arsip.publik.hapus', $item->id) }}" method="POST" class="inline deleteForm">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="bg-[#BB5456] text-white px-2 py-1 rounded hover:bg-[#8B6869] transition text-xs deleteBtn">Hapus</button>
                                </form>
                            </div>
                        </div>
                        @empty
                    ...
                    @if ($arsipPublik->isEmpty())
                        <div class="text-center text-gray-500 py-4 hidden md:block">Tidak Ada Data Publik</div>
                    @endif
                </div>
            </div>
    ...
    {{-- Tambahkan Modal Hapus yang sebelumnya hilang dari kode Anda --}}
    <div 
        id="deleteModal" 
        class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center transition-opacity duration-300 opacity-0 z-[10000]"
        onclick="if(event.target.id === 'deleteModal') closeModal()"
    >
        <div 
            id="modalContent" 
            class="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm transform scale-90 transition-transform transition-opacity duration-300 opacity-0"
        >
            <h3 class="text-xl font-bold mb-3 text-red-600">Konfirmasi Hapus</h3>
            <p class="mb-5 text-gray-700">Apakah Anda yakin ingin menghapus data arsip ini? Tindakan ini tidak dapat dibatalkan.</p>
            <div class="flex justify-end space-x-3">
                <button type="button" id="cancelBtn" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 transition">Batal</button>
                <button type="button" id="confirmBtn" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition">Hapus</button>
            </div>
        </div>
    </div>

    <script>
        // Delete modal logic (single)
        ...
        new TomSelect("#judulSelect", {
            create: false, // TIDAK mengizinkan penambahan opsi baru
            sortField: { field: "text", direction: "asc" },
            placeholder: "Cari Judul...",
            // PENTING: Fitur pencarian/penyaringan sudah aktif secara default
            // Anda hanya perlu mengklik box Tom Select dan mulai mengetik.
        });

        // TomSelect untuk unit pengolah
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const toastWrap = document.querySelector('#toast > div');
            if (toastWrap) {
                toastWrap.classList.remove('opacity-0');
                toastWrap.classList.add('opacity-100');
                setTimeout(() => {
                    toastWrap.classList.add('opacity-0');
                    setTimeout(() => {
